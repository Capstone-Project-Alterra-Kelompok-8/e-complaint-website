...
import { deleteNews } from "../../services/newsSlice";
export default function CardNews({
  ...
  const handleDeleteClick = async () => {
    ...
    if (result.isConfirmed) {
      try {
        // Dispatch action to delete news
        await dispatch(deleteNews(id));

        // Optionally, you can add a success message here
        Swal.fire(
          "Terhapus!",
          "Berita beserta komentarnya telah dihapus.",
          "success"
        );
      } catch (error) {
        // Handle error if deletion fails
        Swal.fire("Error!", "Gagal menghapus berita dan komentar.", "error");
      }
    }
  };
    ...
    <div className="lg:w-[486px] h-auto px-5 py-8 bg-slate-50 rounded-2xl flex flex-col shadow">
      ...
      <div className="flex gap-6 mt-auto items-center justify-between">
        <div className="flex gap-6">
          <button
            className="bg-info-3 text-white px-6 py-2.5 rounded shadow"
            onClick={handleDetailClick}
          >
            Detail
          </button>
          <button
            className="bg-error-3 text-white px-6 py-2.5 rounded shadow"
